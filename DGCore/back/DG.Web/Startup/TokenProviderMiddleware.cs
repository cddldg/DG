...
namespace DG.Web
{
    public class TokenProviderMiddleware
    {
        ...
        /// <returns></returns>
        public async Task Invoke(HttpContext context, IUsersService usersService, IMemoryCache cache)
        {
        ...
        private async Task GenerateAuthorizedResult(HttpContext context)
        {
            ...
            var identity = await GetIdentity(username, password,context);
            if (identity == null)
            {
                await ReturnBadRequest(context);
                return;
        ...
        /// <returns></returns>
        private Task<ClaimsIdentity> GetIdentity(string username, string password, HttpContext context)
        {
            var isValidated = _usersService.CheckUsers(username, password); 
            if (isValidated.IsError==YesNo.No)
            {
        ...
        private async Task ReturnBadRequest(HttpContext context)
        {
            ...
            {
               IsError=YesNo.Yes,
               ErrorCode = ErrorCode.AuthorizeError,
               ErrorMessage = "认证失败"
            };
            await context.Response.WriteAsync(JsonConvert.SerializeObject(rest));
        }
        ...
        private string GetJwt(string username)
        {
            ...
            var response = new JWTOutput()
            { 
                Access_Token = encodedJwt,
                Expires_In = (int)_options.Expiration.TotalSeconds
            };
            Result<JWTOutput> rest = new Result<JWTOutput>
            {
                Data = response
            };
            //保存进缓存
            //_cache.Set<Result>(ConstNames.CacheToken, rest);

            
            return JsonConvert.SerializeObject(rest, new JsonSerializerSettings { Formatting = Formatting.Indented });
        }
    }
}
